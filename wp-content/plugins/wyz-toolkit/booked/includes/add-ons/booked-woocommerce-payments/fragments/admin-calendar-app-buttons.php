...
<?php if ($post_status != 'publish' && $post_status != 'future'): ?>
	<button data-appt-id="<?php echo $appt_id ?>" class="approve button button-primary"><?php echo __('Approve Appointment', 'wyzi-business-finder') ?></button>
<?php endif; ?>
	...
	<span class="booked-wc_status-text awaiting">
		...
		if ( $appointment->order_id ) {
			...
			if (current_user_can('manage_booked_options')) :
				echo '<button data-appt-id="'.$appt_id.'" class="mark-paid button">'.__('Mark as Paid', 'wyzi-business-finder').'</button>';
			...
			echo '<span>' . __('Awaiting Payment', 'wyzi-business-finder') . '</span>';
...
<?php else:
	...
	echo '<span class="booked-wc_status-text paid">';
		...
		if (current_user_can('manage_booked_options') && $appointment->order_id != 'manual') :
			echo '<a target="_blank" href="' . admin_url('/post.php?post=' . $appointment->order_id . '&action=edit') . '">' . __('Paid', 'wyzi-business-finder') . '</a>';
		else :
			echo '<span>' . __('Paid', 'wyzi-business-finder') . '</span>';
		endif;
