...
if ( !$appointment->is_paid && $appointment->payment_status == 'awaiting_checkout' ): ?>
	<a href="#" data-appt-id="<?php echo $appointment_id ?>" class="pay"><?php _e('Pay', 'wyzi-business-finder'); ?></a>
<?php endif ?>

<a href="<?php echo $calendar_link ?>" data-app-calendar="<?php echo $calendar_link ?>" data-appt-id="<?php echo $appointment_id ?>" class="edit"><?php _e('Change Date', 'wyzi-business-finder'); ?></a>
...
<?php if ( !get_option('booked_dont_allow_user_cancellations',false) ) {
	...
	if ( $appt_date_time >= $date_to_compare) {
		if (!$appointment->is_paid || $appointment->is_paid && $appointment->order_id == 'manual'){
			echo '<a href="#" data-appt-id="'.$appointment_id.'" class="cancel">'.__('Cancel Appointment','wyzi-business-finder').'</a>';
		}
	}
}