...
<div class="booked-form booked-scrollable">
	...
	<p class="booked-title-bar"><small><?php echo __('Update Appointment Date', 'wyzi-business-finder'); ?></small></p>
	...
	<form action="" method="post" id="newAppointmentForm" data-calendar-id="<?php echo $calendar_id; ?>">
		...
		<p><?php echo sprintf(__('Please confirm that you would like to change the appointment date for %s to the following:', 'wyzi-business-finder'), $user_nickname); ?></p>
		...
		<div class="field">
			<?php if (!$reached_limit): ?>
				<input type="submit" id="submit-edit-request-appointment" class="button button-primary" value="<?php _e('Update Appointment Date', 'wyzi-business-finder'); ?>">
				<button class="cancel button"><?php _e('Cancel', 'wyzi-business-finder'); ?></button>
			<?php else: ?>
				<button class="cancel button"><?php _e('Okay', 'wyzi-business-finder'); ?></button>
			<?php endif; ?>
		</div>
