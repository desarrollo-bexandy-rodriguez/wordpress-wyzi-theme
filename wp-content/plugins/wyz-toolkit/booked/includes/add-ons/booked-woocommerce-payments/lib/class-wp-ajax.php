...
class Booked_WC_Ajax {
	...
	public function load_product_variations() {
		...
		try {
			...
			$message = __('An error has occur.', 'wyzi-business-finder');
	...
	public function add_appointment_to_cart() {
		...
		if ( !$app_id ) {
			$response->add_message(__('Appointment ID is not defined.', 'wyzi-business-finder'));
			$response->create();
		}
		...
		$response->add_message(__('Appointment has been added to the cart', 'wyzi-business-finder'));
