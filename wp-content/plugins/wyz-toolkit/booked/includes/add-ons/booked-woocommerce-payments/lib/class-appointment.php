...
class Booked_WC_Appointment {
	...
	public static function get( $post_id=null ) {
		if ( !is_integer($post_id) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'wyzi-business-finder'), 'Booked_WC_Appointment::get($post_id)', gettype($post_id) );
	...
	protected function get_timeslot_text() {
		...
		if ( !empty($this->timeslot) ):
			...
			if ($timeslots[0] == '0000' && $timeslots[1] == '2400') {
				$timeslotText = $day_year . ' (' . esc_html__('All day','wyzi-business-finder') . ')';
			} else if ( !$hide_end_times ) {
				$timeslotText = sprintf(__('from %1$s to %2$s on %3$s', 'wyzi-business-finder'), $time_start, $time_end, $day_year);
			} else if ( $hide_end_times ) {
				$timeslotText = sprintf(__('at %1$s on %2$s', 'wyzi-business-finder'), $time_start, $day_year);
			} else {
				$timeslotText = 'N/A';
			}
