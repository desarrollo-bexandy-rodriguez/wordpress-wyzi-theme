...
class Booked_WC_Order_Item_Hooks {
	...
	public static function woocommerce_attribute_label($label, $name, $product=null) {
		...
		if ( preg_match('~custom_field\d+$~', strtolower($label)) ) {
			$label = __('Form Field', 'wyzi-business-finder');
			...
			$label = __('Calendar', 'wyzi-business-finder');
		} else if ( BOOKED_WC_PLUGIN_PREFIX . 'appointment_assignee_name' === strtolower($label) ) {
			$label = __('Booking Agent', 'wyzi-business-finder');
		}
		...
		$label = __('Appointment', 'wyzi-business-finder');
	...
	public static function pre_kses($string, $allowed_html, $allowed_protocols) {
		if ( $string===__('Appointment', 'wyzi-business-finder') ) {
			return $string;
		}
