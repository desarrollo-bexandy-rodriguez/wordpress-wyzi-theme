...
class Booked_WC_Post_Status {
	...
	public function append_post_status_list() {
		...
		if( $post && $post->post_type===BOOKED_WC_POST_TYPE ) {
			...
			if( $post->post_status===$post_status ) {
				...
				$label = '<span id=\"post-status-display\"> ' . __('Awaiting Payment', 'wyzi-business-finder') . '</span>';
			}

			echo '
				<script type="text/javascript">
					jQuery(document).ready(function($){
						$("select#post_status").append("<option value=\"' . $post_status . '\" ' . $complete . '>' .  __('Awaiting Payment', 'wyzi-business-finder') . '</option>");
	...
	public function display_post_states( $states ) {
		...
		if($arg!==$post_status && $post->post_status===$post_status) {
			return array(__('Awaiting Payment', 'wyzi-business-finder'));
		}
