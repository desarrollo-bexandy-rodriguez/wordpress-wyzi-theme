...
class Booked_WC_Order {
	...
	public static function get( $order_id=null ) {
		if ( !is_integer($order_id) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'wyzi-business-finder'), 'Booked_WC_Order::get($order_id)', gettype($order_id) );
		...
		foreach ($cart_appointments['ids'] as $app_id) {
			...
			foreach ($appointment->products as $product) {
				...
				if ( !in_array($check_key, $cart_appointments['extended']) ) {
					$message = sprintf( __('Appointment "%s" and cart products do not match. Please make sure that all appointment products are available in the cart.', 'wyzi-business-finder'), $appointment->timeslot_text );
					throw new Exception( $message );
				}
			}
		}
