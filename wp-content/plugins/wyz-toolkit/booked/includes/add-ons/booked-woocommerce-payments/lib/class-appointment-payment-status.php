...
class Booked_WC_Appointment_Payment_Status {
	...
	public function __construct($app_id) {
		if ( !is_integer($app_id) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'wyzi-business-finder'), 'new Booked_WC_Appointment_Payment_Status::get($app_id)', gettype($app_id) );
			throw new Exception($message);
		} else if ( $app_id===0 ) {
			$message = sprintf( __('%s invalid ID is given. %s', 'wyzi-business-finder'), 'new Booked_WC_Appointment_Payment_Status::get($app_id)', '$app_id=0' );
			throw new Exception($message);
		}
		...
		$this->payment_status_text = __('Awaiting Payment', 'wyzi-business-finder');
	...
	public function set_statuses() {
		...
		if ( $this->order_id && $this->order_id === 'manual' || $this->order_id && $this->order_obj->order->get_status() === 'wc-completed' || $this->order_id && $this->order_obj->order->get_status() === 'completed' ) {
			...
			$this->payment_status_text = __('Order Paid', 'wyzi-business-finder');
			...
			$this->payment_status_text = __('Order ', 'wyzi-business-finder') . $this->order_obj->order->post_status_text;
		}
